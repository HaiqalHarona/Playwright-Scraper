...
def _build_full_url(href):
    ...
    if href.startswith("/"): return "https://www.lazada.sg" + href
    return href


def _extract_grid_data(card, index, matched_selector):
    try:
        name = _extract_text(card, [".RfADt", "[data-qa-locator='product-item'] span", "span"])
        ...
        image = _extract_attribute(card, "img", "src") or _extract_attribute(card, "img", "data-src")
...
def _save_results(products, url):
    ...
    output_dir   = os.path.join(project_root, "data/scrapes")
    os.makedirs(output_dir, exist_ok=True)
    readable_time = datetime.now().strftime('%d-%m-%Y_%H-%M')
    
    # 1. Save Full Results
    full_path = os.path.join(output_dir, f"lazada_scrape-{readable_time}.json")
    with open(full_path, "w", encoding="utf-8") as f:
        json.dump({"scraped_at": datetime.now().isoformat(), "source": url, "products": products}, f, indent=2, ensure_ascii=False)

    # 2. Save Filtered (In Stock Only) Results
    in_stock_products = [p for p in products if p.get("is_out_of_stock") is False]
    in_stock_path = os.path.join(output_dir, f"lazada_in_stock-{readable_time}.json")
    with open(in_stock_path, "w", encoding="utf-8") as f:
        json.dump({"scraped_at": datetime.now().isoformat(), "count": len(in_stock_products), "products": in_stock_products}, f, indent=2, ensure_ascii=False)

    print(f"-> [Lazada] Full results saved to: {full_path}")
    print(f"-> [Lazada] In-stock results saved to: {in_stock_path} ({len(in_stock_products)} items)")
...
def scrape_item_data(page, url):
    ...
    while True:
        ...
        for i, card in enumerate(cards, start=len(grid_products) + 1):
            ...
            if product: grid_products.append(product)

        next_btn = page.query_selector("li.ant-pagination-next")
        if next_btn and "ant-pagination-disabled" not in (next_btn.get_attribute("class") or ""):
